...
int binarySearch_helper(int *input,int val,int si,int ei)
{
    if(si>ei)
    {
        return -1;
    ...
    if(input[mid]>val)
    {
        return binarySearch_helper(input,val,si,mid-1);
    }
    else if(input[mid]<val)
    {
        return binarySearch_helper(input,val,mid + 1,ei);
    }
    else 
    {
        return mid;
...
int binarySearch(int *input, int n, int val)
{
    ...
    return binarySearch_helper(input,val,0,n-1);
}
