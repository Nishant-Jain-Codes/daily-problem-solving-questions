...
int replaceWithLargerNodesSum_sol(BinaryTreeNode<int>* root, int sum)
{
    if(root==NULL)
    {
        return sum;
    }
    sum = replaceWithLargerNodesSum_sol(root->right,sum);
    sum+=root->data;
    root->data=sum;
    sum = replaceWithLargerNodesSum_sol(root->left,sum);
    return sum;
}
void replaceWithLargerNodesSum(BinaryTreeNode<int> *root) {
    // Write your code here
    int ans = replaceWithLargerNodesSum_sol(root,0);
}
